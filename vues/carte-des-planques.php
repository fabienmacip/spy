...
?>
<!-- #####################  DEBUT PLANQUE ########################## -->
<div class="card w-100 border-info mx-auto mt-3">
    ...
    <div class="card-body">
        <!-- Liste des PLANQUES -->
        <?php 
        if (is_null($planques) || empty($planques)):
        ?>
                   Aucune planque...
        <?php else: ?>
                    <?php 
                        ...
                        foreach ($planques as $planque): ?>
                            <div id="unePlanque<?= $planque->getId() ?>" class="sous-carte p-1 my-1 rounded border border-success">     
                                ...
                                <?= $planque->getCode() ?>
                                <button class="btn rounded lesboutons" onclick=confirmeDeletePlanque(<?php echo $planque->getId().",\"".$planque->getCode()."\",".$mission->getId()."" ?>)>
                                ...
                                <ul class="list-group list-group-flush text-start">
                                    ...
                                    <li class="list-group-item">Type : <span class="text-secondary"><?= $planque->getType() ?></span></li>
                                </ul>
                            </div>
                        <?php endforeach; ?>
